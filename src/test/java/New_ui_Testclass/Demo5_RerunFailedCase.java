...
@Listeners(Demo5_RerunFailedCase.class)
public class Demo5_RerunFailedCase implements ITestListener {
    ...
    public void onFinish(ITestContext context) {
        List<ITestResult> retryList = new ArrayList<>(failedTestResults);
        int retryCount = 0; // Initialize retry count outside the loop
        for (ITestResult failedResult : retryList) {
            while (retryCount <= maxRetryCount) {
                System.out.println("Failed test case running (Retry Attempt: " + retryCount + ")");
                ...
                retryTestNG.run();
                System.out.println("After ---> " + "retryTestNG.run()");

                if (failedTestResults.removeIf(r -> r.isSuccess())) {
                    // Test passed in retry, no need to retry further
                    break;
                }

                // Check if the test case is still failed after retry
                boolean stillFailed = failedTestResults.stream()
                        .anyMatch(r -> r.getTestClass().getRealClass().equals(failedTestClass));
                if (stillFailed) {
                    retryCount++; // Increment retry count only if the test case is still failed
                } else {
                    break; // Break the loop if the test case passes in retry
                }
            }
            // Move this line out of the while loop to retain the incremented value
            retryCount = 0; // Reset retry count for the next test case
        }
    }
